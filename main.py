...
initial_components = np.array([[1, 2, 3, 4], [4, 3, 2,6], [0, 0, 0,0]])
...
for run in range(0,initial_components.shape[0]):
    ...
    trajdata.append([A1, A2, lamb, eps, dt, N])

print(trajdata)

def label(A1, A2, lamb, eps, dt, N):
    return ('traj_(' + str(A1) + ',' + str(A2) + ')_' + str(lamb) + '_' + str(eps) + '_' + str(dt) + '_' + str(N) + '.npy')
...
def f(A, lamb, eps):
    ...
    s = np.ones((2, 2))
    array[0] = 1j * lamb * A1 + eps * A2 + 1j * A1 * (s[0, 0] * np.abs(A1) ** 2 + s[0, 1] * np.abs(A2) ** 2)
    array[1] = eps * A1 + 1j * A2 * (s[1, 0] * np.abs(A1) ** 2 + s[1, 1] * np.abs(A2) ** 2)
    return array
...
def compute_traj(trajdata):  # calculates and stores trajectories, given a list of their data, returns dictionary of trajectories
    trajs = dict()
    for [A1, A2, lamb, eps, dt, N] in trajdata:

        traj = np.zeros((N, 2), dtype=np.complex64)
        traj[0, :] = A1, A2
        for i in range(0, N - 1):
            a1 = dt * f(traj[i, :], lamb, eps)
            a2 = dt * f(traj[i, :] + a1 / 2, lamb, eps)
            a3 = dt * f(traj[i, :] + a2 / 2, lamb, eps)
            a4 = dt * f(traj[i, :] + a3, lamb, eps)
            traj[i + 1, :] = traj[i, :] + (a1 + 2 * a2 + 2 * a3 + a4) / 6

        name = label(A1, A2, lamb, eps, dt, N)
        trajs[name] = traj
